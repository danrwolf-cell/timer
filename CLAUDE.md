...
| `src/import/route-scan-result.ts` | Shared `ExtractResponse` shape between the direct and self-hosted paths, so `ScanReviewScreen` treats them identically. |
...
| `src/screens/RouteCameraScreen.tsx` | Route-sheet capture: viewfinder + photo-library shortcut, either path replaces itself with `ScanReviewScreen`. |
| `src/screens/ScanReviewScreen.tsx` | Photo/PDF route-sheet import: extraction runs immediately on the file passed in via `route-scan-direct.ts` (rider's own key); shows live progress, per-checkpoint pass/fail, save. |
